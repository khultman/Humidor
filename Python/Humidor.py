...
class Humidor(object):
	...
	def get_sensor_data(self):
		...
		for i in range(0, self.sensors):
			...
			self._log.debug("Channel {0}, temp_c {1}, temp_f {2}, humidity {3}".format(i, temp_c[i], temp_f[i], humidity[i]), extra={'instance_id': "{0}.{1}".format(self.__class__.__name__,__name__)})
